...
These thresholds are about attention, not context capacity: a large context window lets more source fit, but it does not make one careful reading pass of 500K characters any more reliable. If the user wants to push past them, that is their judgment call, and the report should say so.
...
Read `~/.claude/skills/review-common/interview.md` for prior-review
carry-over and the GitHub-issue question before interviewing the user below.
...
Also ask the GitHub-issue question from `~/.claude/skills/review-common/interview.md`.
...
Read `~/.claude/skills/review-common/plan-file.md` for the plan schema, release chunks, and the closing briefing before writing the plan. Write the plan to `INTEGRATION_REVIEW_PLAN.md` in the project root. In each chunk's description, give the file:line and what the chunk fixes or answers, and reference the originating tier and finding inline.
