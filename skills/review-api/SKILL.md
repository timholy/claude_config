...
Read `~/.claude/skills/review-common/plan-file.md` for the plan schema,
release chunks, and the closing briefing before writing the plan. Write the
plan to `API_REVIEW_PLAN.md` in the project root. In each chunk's description,
give the current signature → proposed signature, with rationale by analogy to
Base, and reference the originating tier / convention / function name inline.
